...
<div class="main-container container-fluid pt-4">
    ...
    <div class="partner_container">
        ...
        <div class="card wide-card w-100 d-flex align-items-center p-4" id="partner_{{$partner->id}}">
            <div class="row w-100 h-100">
                <div class="col-md-1">
                    <img src="@if($partner->thumbnail){{ asset('storage/partner/small/'.$partner->thumbnail->name) }}@endif" alt="logo">
                </div>
                <div class="col-md-7">
                    <div class="card-content w-100 d-flex" >
                        <div class="card-collection ps-4 mb-0">
                            <p class="title">Name</p>
                            <p class="content">{{ $partner->name }}</p>
                        </div>
                        <div class="card-collection ps-5 mb-0">
                            <p class="title">Add date:</p>
                            <p class="content">{{ date("d F, Y", strtotime($partner->created_at));  }}</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-4 h-100">
                    <div class="actions w-100 d-flex justify-content-end align-items-center h-100">
                        <button class="btn-secondary mx-2 delete-btn" data-bs-toggle="modal" data-bs-target="#deleteModel"  route="{{ route('partner.delete',['partner'=>$partner->id]) }}" delete_id="{{ $partner->id }}" callback="deletepartner">
                            Delete
                        </button>
                    </div>
                </div>
...
<script>
    ...
    $(window).scroll(function(){
        ...
        if(allowLoad && ! allLoaded) {
            ...
            $.ajax({
                url: `/admin-panal/$partners/load?page=${page}`,
                beforeSend: () => {
                    allowLoad = false;
                },
                success: (e) => {
                    ...
                    payload.forEach((element) => {
                        $('.partner_container').append(build(element))
                    })
                }
            })
        }
    ...
    function addpartner(e) {
        ...
        $('.partner_container').append(build(payload))
    }

    function build (payload) {
        let deleteUrl = '{{ route("partner.delete", ":id") }}';
        console.log(payload);
        ...
        return `
            <div class="card wide-card w-100 d-flex align-items-center p-4">
                <div class="row w-100 h-100">
                    <div class="col-md-1">
                        <img src="${image}" class="display w-100" alt="partner">
                    </div>
                    <div class="col-md-7">
                        <div class="card-content w-100 d-flex" >
                            <div class="card-collection ps-4 mb-0">
                    ...
                    <div class="col-md-4 h-100">
                    <div class="actions w-100 d-flex justify-content-end align-items-center h-100">
                        <form class="form partner-form ajax-form" action="${deleteUrl}" method="partner">
                            <button class="btn">
