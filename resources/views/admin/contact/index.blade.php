...
<div class="main-container container-fluid pt-4">
    <div class="card custom-card">
        <div class="card-body">
            ...
            <div>
                <form class="form contact-form ajax-form" action="{{ route('contact.store') }}" method="post" refreshAfterSend>
                    <div class="form-group mb-0">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <input class="form-control" id="name" type="text" placeholder="Value" name="value">
    ...
    <div class="contact_container">
        ...
        <div class="card wide-card w-100 d-flex align-items-center p-4" id="contact_{{$contact->id}}">
            <div class="row w-100 h-100">
                <div class="col-md-8">
                    <div class="card-content w-100 d-flex" >
                        <div class="card-collection ps-4 mb-0">
                            <p class="title">Type</p>
                            <p class="content">{{ $contact->type }}</p>
                        </div>
                        <div class="card-collection ps-4 mb-0">
                            <p class="title">Value</p>
                            <p class="content">{{ $contact->value }}</p>
                        </div>
                        <div class="card-collection ps-5 mb-0">
                            <p class="title">Add date:</p>
                            <p class="content">{{ date("d F, Y", strtotime($contact->created_at));  }}</p>
                        </div>
                    </div>
                </div>
                <div class="col-md-4 h-100">
                    <div class="actions w-100 d-flex justify-content-end align-items-center h-100">
                        <button class="btn-secondary mx-2 delete-btn" data-bs-toggle="modal" data-bs-target="#deleteModel"  route="{{ route('contact.delete',['contact'=>$contact->id]) }}" delete_id="{{ $contact->id }}" callback="deletecontact">
                            Delete
                        </button>
                    </div>
                </div>
...
<script>
    ...
    $(window).scroll(function(){
        ...
        if(allowLoad && ! allLoaded) {
            ...
            $.ajax({
                url: `/admin-panal/$contacts/load?page=${page}`,
                beforeSend: () => {
                    allowLoad = false;
                },
                success: (e) => {
                    ...
                    payload.forEach((element) => {
                        $('.contact_container').append(build(element))
                    })
                }
            })
        }
    ...
    function addcontact(e) {
        ...
        $('.contact_container').append(build(payload))
    }

    function build (payload) {
        let deleteUrl = '{{ route("contact.delete", ":id") }}';
        console.log(payload);
        ...
        return `
            <div class="card wide-card w-100 d-flex align-items-center p-4">
                <div class="row w-100 h-100">
                    <div class="col-md-1">
                        <img src="${image}" class="display w-100" alt="contact">
                    </div>
                    <div class="col-md-7">
                        <div class="card-content w-100 d-flex" >
                            <div class="card-collection ps-4 mb-0">
                    ...
                    <div class="col-md-4 h-100">
                    <div class="actions w-100 d-flex justify-content-end align-items-center h-100">
                        <form class="form contact-form ajax-form" action="${deleteUrl}" method="contact">
                            <button class="btn">
